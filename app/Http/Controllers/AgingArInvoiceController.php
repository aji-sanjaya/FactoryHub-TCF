...
class AgingArInvoiceController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->has('c_bpartner_id') && $request->c_bpartner_id) {
            $selectedCustomer = DB::connection('idempiere')->table('c_bpartner')
        ...
        // Query for AR Invoices (issotrx = 'Y' means AP/Customer Invoice)
        $query = CInvoice::query()
            ...
            ->where('issotrx', 'Y') // AR Invoice (customer invoice)
        ...
        if ($request->ajax()) {
            return view('pages.aging-ar-invoice-report.partials.table', compact('invoices'));
        }

        return view('pages.aging-ar-invoice-report.index', compact(
            'invoices',
            'count030',
            'count3160',
            'count6190',
