...
class MenuHelper
{
    ...
    public static function getWelcomeRootItems(?int $sortOrderMin = null, ?int $sortOrderMax = null)
    {
        ...
        foreach ($menus as $menu) {
            $itemsById[$menu->tcf_web_portal_menus_id] = self::makeMenuItem($menu);
        }

        foreach ($menus as $menu) {
            $menuId = $menu->tcf_web_portal_menus_id;
            ...
            $path = $menus
                ->filter(function ($menu) use ($level2Menu) {
                    return self::isLevel3Menu($menu)
                        && (int) $menu->parent_id === (int) $level2Menu->tcf_web_portal_menus_id
                        && filled($menu->path);
                })
                ->sortBy('sort_order')
                ->pluck('path')
    ...
    private static function fetchAuthorizedMenus(?int $sortOrderMin = null, ?int $sortOrderMax = null)
    {
        ...
        $menusQuery = \Illuminate\Support\Facades\DB::connection('idempiere')
            ->table('tcf_web_portal_menus as m')
            ->join('tcf_web_portal_role_menus as rm', 'm.tcf_web_portal_menus_id', '=', 'rm.tcf_web_portal_menus_id')
    ...
    {
        $menusById = $menus->keyBy('tcf_web_portal_menus_id');
    ...
    {
        if (self::isLevel1Menu($menu)) {
            return (int) $menu->tcf_web_portal_menus_id;
        }
    ...
    {
        $item = [
            'id' => $menu->tcf_web_portal_menus_id,
            'name' => $menu->name,
