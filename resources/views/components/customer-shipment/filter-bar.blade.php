<div x-data="{ 
    ...
    applyFilters() {
        ...
        // Convert status array to comma-separated string for URL parameters
        const payload = { ...this.filters, status: this.filters.status.join(',') };
        document.dispatchEvent(new CustomEvent('filter-applied', { detail: payload }));
    },
    resetFilters() {
        this.filters = {
            description: '',
    ...
    <div class="flex items-center gap-3 relative">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <div class="flex gap-2">
                        <input type="date" x-model="filters.date_movement_start"
                            class="w-full rounded-lg border border-gray-300 bg-transparent px-3 py-2 text-sm text-gray-800 focus:border-brand-500 focus:outline-none dark:border-gray-700 dark:text-white" />
                        <span class="self-center text-gray-500">-</span>
                        <input type="date" x-model="filters.date_movement_end"
                            class="w-full rounded-lg border border-gray-300 bg-transparent px-3 py-2 text-sm text-gray-800 focus:border-brand-500 focus:outline-none dark:border-gray-700 dark:text-white" />
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="flex flex-col gap-2">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" value="DR" x-model="filters.status" class="w-4 h-4 rounded border-gray-300 text-brand-500 focus:ring-brand-500">
                            <span class="text-sm text-gray-700 dark:text-gray-300">Drafted</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" value="CO" x-model="filters.status" class="w-4 h-4 rounded border-gray-300 text-brand-500 focus:ring-brand-500">
                            <span class="text-sm text-gray-700 dark:text-gray-300">Completed</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" value="RE" x-model="filters.status" class="w-4 h-4 rounded border-gray-300 text-brand-500 focus:ring-brand-500">
                            <span class="text-sm text-gray-700 dark:text-gray-300">Reversed</span>
                        </label>
                    </div>
                </div>
            </div>
        ...
        <!-- Create Button -->
        <a href="{{ route('customer-shipment.index', ['document_id' => 'new']) }}"
