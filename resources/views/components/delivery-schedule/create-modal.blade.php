<div x-data="{
    ...
    init() {
        document.addEventListener('open-create-modal', () => {
            ...
            this.form = {
                warehouse_id: '',
                date_required: new Date().toISOString().slice(0, 10), // Default today
                description: ''
            };
        });
    },
    ...
    submit() {
        this.loading = true;
        axios.post('{{ route('delivery-schedule.store') }}', this.form)
            .then(res => {
                this.isOpen = false;
                ...
                document.dispatchEvent(new CustomEvent('delivery-schedule-created')); 
            })
            .catch(err => {
                console.error(err);
                alert('Failed to create delivery-schedule');
            })
            .finally(() => {
                this.loading = false;
            });
                            ...
                            <div class="mt-4 space-y-4">
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Date
                                        Required</label>
                                    <input type="date" x-model="form.date_required"
                                        class="mt-1 block w-full rounded-lg border border-gray-300 bg-transparent px-3 py-2 text-sm text-gray-800 focus:border-brand-500 focus:outline-none dark:border-gray-700 dark:text-white" />
                                </div>
