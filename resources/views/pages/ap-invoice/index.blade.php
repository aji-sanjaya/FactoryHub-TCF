...
@section('content')
    <div class="main-content group-data-[sidebar-size=lg]:xl:ml-[322px]">
        ...
        <!-- Summary Cards -->
        <x-ap-invoice.summary-cards :countAll="$countAll" :countDraft="$countDraft" :countInProgress="$countInProgress"
            :countCompleted="$countCompleted" />

        <!-- Action / Filter Bar -->
        <x-ap-invoice.filter-bar />

        <!-- Table -->
        <div id="invoice-table-wrapper">
            <x-ap-invoice.invoice-table :invoices="$invoices" />
        </div>
    ...
    @push('scripts')
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                const fetchData = (url = '{{ route("ap-invoice.index") }}', extraParams = {}) => {
                ...
                tableWrapper.addEventListener('click', function (e) {
                    ...
                    if (link && link.href && link.href.includes('page=')) {
                        ...
                        fetchData('{{ route("ap-invoice.index") }}', { page });
                    }
                });
            });
        </script>
