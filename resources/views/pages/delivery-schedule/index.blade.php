...
@section('content')
    <div class="main-content group-data-[sidebar-size=lg]:xl:ml-[322px]">
        ...
        <!-- Summary Cards -->
        <x-delivery-schedule.summary-cards :countAll="$countAll" :countDraft="$countDraft" :countInProgress="$countInProgress"
            :countCompleted="$countCompleted" />

        <!-- Action / Filter Bar -->
        <x-delivery-schedule.filter-bar />

        <!-- Table -->
        <div id="delivery-schedule-table-wrapper">
            <x-delivery-schedule.delivery-schedule-table :delivery-schedules="$deliverySchedules" />
        </div>

        <!-- Create Modal -->
        <x-delivery-schedule.create-modal />
    ...
    @push('scripts')
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                const tableWrapper = document.getElementById('delivery-schedule-table-wrapper');

                let debounceTimer;
                let currentFilters = {}; // Store active filters

                // Function to fetch data (URL optional)
                const fetchData = (url = '{{ route("delivery-schedule.index") }}') => {
                    const query = searchInput.value;
                    const wrapper = document.getElementById('delivery-schedule-table-wrapper');
                ...
                // Listen for Delivery Schedule Created Event
                document.addEventListener('delivery-schedule-created', function () {
                    console.log('Delivery Schedule created event caught, refreshing table...');
                    fetchData();
                });
