...
@section('content')
    <div class="main-content group-data-[sidebar-size=lg]:xl:ml-[322px]">
        ...
        {{-- Summary Cards --}}
        <x-ar-receipt.summary-cards :countAll="$countAll" :countDraft="$countDraft" :countInProgress="$countInProgress"
            :countCompleted="$countCompleted" />

        {{-- Action / Filter Bar --}}
        <x-ar-receipt.filter-bar />

        {{-- Table --}}
        <div id="payment-table-wrapper">
            <x-ar-receipt.payment-table :payments="$payments" />
        </div>
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                const fetchData = (url = '{{ route("ar-receipt.index") }}', extraParams = {}) => {
                ...
                tableWrapper.addEventListener('click', function (e) {
                    ...
                    if (link && link.href && link.href.includes('page=')) {
                        ...
                        fetchData('{{ route("ar-receipt.index") }}', { page });
                    }
                });
            });
        </script>
