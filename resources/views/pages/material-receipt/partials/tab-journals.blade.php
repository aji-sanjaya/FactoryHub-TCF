    ...
    <div class="flex items-center justify-between mb-4">
        <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                View accounting facts associated with this material receipt.
            </p>
        </div>
        <div class="flex gap-2">
            <a href="{{ route('material-receipt.export-journals', ['id' => $docIdParam]) }}"
...
<script>
    window.repostReceipt = function () {
        const docIdParam = '{{ $docIdParam }}';

        Swal.fire({
            title: 'Re-Post Material Receipt?',
            ...
            if (result.isConfirmed) {
                ...
                axios.post(`{{ url('/create-gr/repost') }}/${docIdParam}`, {
                    _token: '{{ csrf_token() }}'
                }).then(res => {
                    Swal.fire({
                        icon: 'success',
                        title: 'Success!',
                        text: res.data.message || 'Receipt marked for re-posting.',
                        timer: 1500,
