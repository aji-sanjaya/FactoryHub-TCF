        ...
        @forelse($attachments ?? [] as $att)
            @php
                ...
                $previewUrl = (isset($docIdParam) && $attName !== 'Unknown')
                    ? route('material-receipt.attachment.view', ['document_id' => $docIdParam, 'file_name' => $attName])
                    : '';
            @endphp
            ...
            <div class="{{ $previewUrl ? 'cursor-pointer hover:shadow-md' : '' }} group relative border border-gray-200 rounded-lg p-3 transition-shadow dark:border-gray-700 dark:bg-gray-800"
                ...
                <div class="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onclick="event.stopPropagation(); deleteAttachment('{{ $attId ?? addslashes($attName) }}')"
                        class="p-1 bg-white rounded-full shadow text-red-500 hover:text-red-700 hover:bg-red-50 transition-colors cursor-pointer" title="Delete">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                        </svg>
                    </button>
