<div class="mt-5 space-y-6">
    ...
    @if(!$isReadOnly)
        <div class="p-6 border-b">
            <div ondragover="event.preventDefault(); this.classList.add('border-brand-500', 'bg-brand-50');"
                ...
                onclick="document.getElementById('fileInputPettyCash').click()">

                <input type="file" id="fileInputPettyCash" class="hidden" multiple accept=".jpg,.jpeg,.png,.pdf,.xlsx,.xls"
                ...
                <p class="text-xs text-gray-500 mt-1">PNG, JPG, PDF or Excel (MAX. 10MB)</p>
            </div>
        </div>
    @endif
    ...
    @if(!$isReadOnly && count($attachments ?? []) > 0)
        <div class="flex items-center justify-between px-4">
            <div class="flex items-center space-x-2 select-none cursor-pointer"
                onclick="document.getElementById('selectAllAttachments').click()">
                <input type="checkbox" id="selectAllAttachments"
                    class="rounded border-gray-300 text-brand-600 focus:ring-brand-500 w-4 h-4 cursor-pointer"
                    onclick="event.stopPropagation(); toggleSelectAll(this)">
                <span class="text-sm text-gray-700 dark:text-gray-300 font-medium">Select All</span>
            </div>
            <button id="btnDeleteSelected" onclick="deleteSelectedAttachments()"
                class="hidden px-3 py-1.5 bg-red-600 text-white rounded-md text-sm hover:bg-red-700 transition-colors shadow-sm flex items-center gap-1">
        ...
        @forelse($attachments ?? [] as $att)
            @php
                ...
                $previewUrl = (isset($docIdParam) && $attName !== 'Unknown') 
                    ? route('petty-cash-closing.attachment.view', ['document_id' => $docIdParam, 'file_name' => $attName]) 
                    : '';
            @endphp
            ...
            <div class="{{ $previewUrl ? 'cursor-pointer hover:shadow-md' : '' }} group relative border border-gray-200 rounded-lg p-3 transition-shadow dark:border-gray-700 dark:bg-gray-800"
                data-url="{{ $previewUrl }}" data-name="{{ $attName }}"
                ...
                @if(!$isReadOnly)
                    <div class="absolute top-2 left-2 z-10" onclick="event.stopPropagation()">
                        <input type="checkbox"
                            class="attachment-checkbox w-4 h-4 rounded border-gray-300 text-brand-600 focus:ring-brand-500 cursor-pointer shadow-sm bg-white"
                            value="{{ $attId ?? $attName }}" onchange="updateDeleteButtonState()">
                    </div>
                @endif

                <!-- Icon / Preview -->
                <div
                    class="aspect-square bg-gray-100 rounded-md mb-2 flex items-center justify-center overflow-hidden dark:bg-gray-700 pointer-events-none">
                    @if($isImage)
                ...
                <p class="text-xs font-medium text-gray-700 truncate dark:text-gray-300 pointer-events-none"
                    title="{{ $attName }}">
                    {{ $attName }}
                </p>
                ...
                @if(!$isReadOnly)
                    <div class="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onclick="event.stopPropagation(); deleteAttachment('{{ $attId ?? addslashes($attName) }}')"
                            class="p-1 bg-white rounded-full shadow text-red-500 hover:text-red-700 hover:bg-red-50 transition-colors cursor-pointer"
                            title="Delete">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
