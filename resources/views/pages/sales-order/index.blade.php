...
@section('content')
    <div class="main-content group-data-[sidebar-size=lg]:xl:ml-[322px]">
        ...
        <!-- Summary Cards -->
        <x-sales-order.summary-cards :countAll="$countAll" :countDraft="$countDraft" :countInProgress="$countInProgress"
            :countCompleted="$countCompleted" />

        <!-- Action / Filter Bar -->
        <x-sales-order.filter-bar />

        <!-- Table -->
        <div id="sales-order-table-wrapper">
            <x-sales-order.sales-order-table :sales-orders="$salesOrders" />
        </div>

        <!-- Create Modal -->
        <x-sales-order.create-modal />
    ...
    @push('scripts')
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                const tableWrapper = document.getElementById('sales-order-table-wrapper');

                let debounceTimer;
                let currentFilters = {}; // Store active filters

                // Function to fetch data (URL optional)
                const fetchData = (url = '{{ route("sales-order.index") }}') => {
                    const query = searchInput.value;
                    const wrapper = document.getElementById('sales-order-table-wrapper');
                ...
                // Listen for Sales Order Created Event
                document.addEventListener('sales-order-created', function () {
                    console.log('Sales Order created event caught, refreshing table...');
                    fetchData();
                });
