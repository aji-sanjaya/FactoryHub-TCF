@php
    $currentBPartnerId = $salesOrder->c_bpartner_id ?? null;
    $currentBPartnerLocationId = $salesOrder->c_bpartner_location_id ?? null;
    $currentBillBPartnerId = $salesOrder->bill_bpartner_id ?? $currentBPartnerId;
    $currentBillLocationId = $salesOrder->bill_location_id ?? $currentBPartnerLocationId;

    $dateOrdered = $isNew ? now()->format('m-d-Y') : \Carbon\Carbon::parse($salesOrder->dateordered)->format('m-d-Y');
    $datePromised = $isNew ? now()->format('m-d-Y') : \Carbon\Carbon::parse($salesOrder->datepromised)->format('m-d-Y');
    ...
    $grandTotal = $salesOrder->grandtotal ?? 0;
    $totalLines = $salesOrder->totallines ?? 0;
    ...
    $orderReference = $salesOrder->poreference ?? '';
    ...
    $currentPricelistId = $isNew ? $defaultPricelistId : ($salesOrder->m_pricelist_id ?? null);
    $defaultWarehouseId = null;
    if (isset($warehouses) && count($warehouses) > 0) {
        $sortedWh = collect($warehouses)->sortBy('id');
        $defaultWarehouseId = $sortedWh->first()->id;
    }
    $warehouseId = $isNew ? $defaultWarehouseId : ($salesOrder->m_warehouse_id ?? null);
    $defaultOrgId = null;
    if (isset($organizations) && count($organizations) > 0) {
        $sortedOrgs = collect($organizations)->sortBy('id');
        $defaultOrgId = $sortedOrgs->first()->id;
    }
    $currentOrgId = $isNew ? $defaultOrgId : ($salesOrder->ad_org_id ?? null);
    ...
    $currentTaxId = $isNew ? $defaultTaxId : ($salesOrder->c_tax_id ?? null);
    
    // Check if order has lines - tax cannot be changed if lines exist
    $hasOrderLines = false;
    if (!$isNew && $salesOrder->c_order_id) {
        $lineCount = \Illuminate\Support\Facades\DB::connection('idempiere')
            ->table('c_orderline')
            ->where('c_order_id', $salesOrder->c_order_id)
            ->count();
...
<div class="px-8 py-6 max-w-7xl mx-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-4">
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-3 items-center gap-3">
                    ...
                    <div class="col-span-2">
                        @php
                            $currentDocTypeId = $isNew ? 1000032 : ($salesOrder->c_doctypetarget_id ?? 1000032);
                        @endphp
                        <select id="c_doctypetarget_id" {{ $isReadOnly ? 'disabled' : '' }}
                            class="w-full text-sm rounded-lg border-gray-300 focus:border-brand-500 focus:ring-brand-500 dark:bg-gray-900 dark:border-gray-600 {{ $isReadOnly ? 'bg-gray-50 cursor-not-allowed' : '' }}">
                            <option value="1000032" {{ $currentDocTypeId == 1000032 ? 'selected' : '' }}>SO to Shipment</option>
                            <option value="1000029" {{ $currentDocTypeId == 1000029 ? 'selected' : '' }}>SO to DN</option>
                        </select>
                    </div>
                </div> 
    ...
    @if(!$isNew)
        <div class="flex justify-end gap-3 pt-6 border-t border-gray-200 dark:border-gray-700 mt-6">
            ...
            @if(!$isReadOnly)
                <button type="button" onclick="deleteSalesOrder()"
